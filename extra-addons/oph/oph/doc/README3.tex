...
\section{Les passwords}
\label{sec:password}

il y a trois password dans openerp:
\begin{enumerate}
\item admin_password qui se définit dans le opener-server.conf. Il est à admin par défaut.
\item db_password qui se définit aussi dans le opener-server.conf. Je pense que c'est le password du role 'openerp'' dans postgresql.
\item password pour l'administrateur de la database.
\end{enumerate}

Quand on crée une database on rentre:
\begin{enumerate}
\item le \textbf{Master Password} qui le password définie dans le openerp-server.conf sous \textit{admin_password}
\item le \textbf{Admin Password} qui est le password pour le nouvel user admin pour la nouvelle database.  
\end{enumerate}

 
\section{Comment récupérer les password des différents users}
\label{sec:password_user}
su
su postgres
psql -d mydatabasename
select login, password from res_users;

On peut aussi utiliser pgadmin c'est donc dans la table res_users.


\section{Comment supprimer la liste des databases dans l'interface de connerction}
\label{sec:list_database}

openepr-server --no-database-list
